...
const StatistikBeritaView = lazy(() => import('./components/StatistikBeritaView'));
const AdSenseBeritaView = lazy(() => import('./components/AdSenseBeritaView'));
...
// Authentication Handlers
  ...
  // Nama tidak pernah digunakan sebagai identifier.
  useEffect(() => {
    if (!currentUser || currentUser.role === 'admin' || members.length === 0) {
      return;
    }

    const userEmail = currentUser.email?.toLowerCase().trim();

    let matched = currentUser.memberId
      ? members.find((m) => m.id === currentUser.memberId)
      : undefined;

    // Migrasi akun lama: hubungan userId -> member.
    if (!matched) {
      matched = members.find(
        (m) => (m as any).userId === currentUser.id
      );
    }

    // Migrasi akun lama: email -> member.
    if (!matched && userEmail) {
      matched = members.find(
        (m) =>
          typeof (m as any).email === 'string' &&
          (m as any).email.toLowerCase().trim() === userEmail
      );
    }
    ...
    if (currentUser.role === 'admin') {
      ...
      const unsubMembers = onSnapshot(collection(db, COLLECTIONS.MEMBERS), (snapshot) => {
        if (!snapshot.empty) {
          ...
          // Linking akun hanya boleh dilakukan oleh admin.
          void syncMemberAccountLinks(cloudMembers);
        }
      ...
      const unsubPayments = onSnapshot(collection(db, COLLECTIONS.PAYMENTS), (snapshot) => {
        ...
        setPayments((currentPayments: MemberPaymentHistory[]) => {
          const currentByMember = new Map<string, MemberPaymentHistory>(
            currentPayments.map((payment) => [payment.memberId, payment])
          );

          const merged = cloudPayments.map(
            (cloudPayment: MemberPaymentHistory) => {
              const localPayment = currentByMember.get(
                cloudPayment.memberId
              );

              // Tidak ada data lokal -> gunakan Firestore.
              if (!localPayment) {
                return {
                  ...cloudPayment,
                  arisan: {
              ...
              // Gabungkan per bulan agar snapshot Firestore
              // yang datang terlambat tidak menghilangkan perubahan lokal.
              return {
                ...localPayment,
                ...cloudPayment,
                memberId: cloudPayment.memberId,
                arisan: {
                  ...(localPayment.arisan || {}),
                  ...(cloudPayment.arisan || {}),
                },
...
// Keep locally-created member payment records until Firestore
      ...
      // Member.id adalah identitas kanonik untuk:
// members/{memberId} dan payments/{memberId}
      const memberId = currentUser.memberId;

      // Member.id adalah identitas kanonik.
      // Jika belum ada linkage, jangan pernah memakai Firebase UID
      // sebagai pengganti Member.id.
      if (!memberId) {
        console.warn(
          'Akun anggota belum memiliki Member.id kanonik; realtime member/payment tidak dipasang.'
        );
        return;
      }
  ...
  // ============================================================
  // ADMIN ONLY:
  // Hubungkan akun Firebase dengan Member.id kanonik berdasarkan email.
  //
  // users/{FirebaseUID}.memberId
  //          <-> 
  // members/{Member.id}.userId
  //
  // Email hanya digunakan untuk menemukan pasangan akun saat linking.
  // Identitas aplikasi tetap Member.id.
  // ============================================================
  const syncMemberAccountLinks = useCallback(async (memberList: Member[]) => {
    if (currentUser?.role !== 'admin' || memberList.length === 0) {
      return;
    }
    ...
    try {
      ...
      for (const member of memberList) {
        ...
        const { uid, data: userData } = matchedUser;

        // Jangan memindahkan akun yang sudah terhubung
        // ke Member.id lain.
        if (
          userData.memberId &&
          ...
          console.warn(
            `Akun ${email} sudah terhubung ke Member.id ` +
            `${userData.memberId}; tidak dipindahkan ke ${member.id}.`
          );
          continue;
        }

        const memberNeedsLink =
          member.userId !== uid ||
          member.email !== email;
        ...
        const userNeedsLink =
          ...
          (member.phone || '') !== (userData.phoneNumber || '');

        // PENTING:
        // Jika kedua dokumen sudah benar, jangan write lagi.
        // Ini mencegah loop onSnapshot -> write -> onSnapshot.
        if (!memberNeedsLink && !userNeedsLink) {
          continue;
        }
        ...
        console.info(
          `Linked akun ${email} -> Member.id ${member.id}`
        );
      }
    } catch (error) {
      console.warn(
        'Sinkronisasi linkage akun anggota gagal:',
        error
      );
    }
  }, [currentUser?.role]);
  ...
  const handleAddMember = async (memberData: Omit<Member, 'id' | 'no'>) => {
    ...
    const newMember: Member = {
      ...
      id: newId,
      no: newNo,
      email: memberData.email?.trim().toLowerCase() || undefined,
    };
  ...
  const handleUpdateMember = async (updatedMember: Member) => {
    const normalizedMember: Member = {
      ...updatedMember,
      email: updatedMember.email?.trim().toLowerCase() || undefined,
    };

    setMembers((prev) =>
      prev.map((m) =>
        m.id === normalizedMember.id ? normalizedMember : m
      )
    );
    ...
    await syncMemberAccountLinks([normalizedMember]);
  };
  ...
  const getTabTitle = (tab: TabType): string => {
    switch (tab) {
      case 'berita':
        return 'Berita';
      case 'statistik_berita':
        return 'Statistik Berita';
      case 'adsense_berita':
        return 'AdSense Berita';
      case 'dashboard':
        return 'Dashboard';
      case 'user_portal':
        return 'Data Anggota';
