...
export const getStoredChatMessages = (): ChatMessage[] => {
  try {
    ...
    // Jika STORAGE_KEY sudah ada, jadikan sebagai sumber data utama.
    // Termasuk ketika isinya [] karena semua pesan memang sudah dihapus.
    if (saved !== null) {
...
export const deleteChatMessage = (messageId: string): ChatMessage[] => {
  ...
  // Simpan daftar utama, termasuk ketika hasilnya [].
  saveChatMessages(updated);

  // Hapus juga pesan dari riwayat lama chat_history_* agar
  // pesan yang sudah dihapus tidak muncul kembali setelah refresh.
  if (target?.memberId) {
    try {
      const memberKey = `chat_history_${target.memberId}`;
      ...
      if (raw) {
        ...
        if (Array.isArray(history)) {
          ...
          localStorage.setItem(memberKey, JSON.stringify(cleanedHistory));
        }
      }
    } catch (err) {
      console.warn('Gagal membersihkan chat_history setelah menghapus pesan:', err);
    }
  }
