...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      >
        {/* Brand Banner */}
        <div className="flex h-20 items-center gap-3 border-b border-slate-200 dark:border-slate-800 px-5">
          {logoUrl ? (
            <img
              src={logoUrl}
              alt="Logo Paguyuban"
              className="h-11 w-11 object-contain shrink-0 bg-transparent"
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="rounded-sm bg-emerald-600 px-1 py-0.2 text-[9px] font-bold text-white uppercase tracking-wider">
                2026
              </span>
            </div>
            {currentUser?.name && (
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`group flex w-full items-center justify-between rounded-xl px-3.5 py-2 text-[13px] font-medium transition-all duration-150 cursor-pointer ${
                  isActive
        ...
        <div className="border-t border-slate-200 dark:border-slate-800 p-3 space-y-1.5">
          {!isUserRole && onOpenBackup && (
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 py-1 px-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-white font-medium text-[10px] shadow-xs transition-all active:scale-95 cursor-pointer"
            >
              <Cloud className="h-3 w-3" />
              <span className="text-[10px] tracking-tight">Backup Google Drive</span>
            </button>
          )}

          {/* Cloud Database Persistence Badge */}
          <div className="flex items-center justify-between px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-[10px]">
            <div className="flex items-center gap-1 font-semibold text-emerald-800 dark:text-emerald-300">
              <span className={`h-1.5 w-1.5 rounded-full ${isFirebaseSyncing ? 'bg-amber-500 animate-ping' : 'bg-emerald-500 animate-pulse'}`} />
          ...
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/60 p-2.5 border border-slate-200 dark:border-slate-700/60 space-y-1.5">
            <div>
            ...
            <div className="pt-1.5 border-t border-slate-200/80 dark:border-slate-700/80 text-[9px] text-slate-500 dark:text-slate-400 text-center flex flex-col items-center justify-center gap-0.5">
              <p className="font-semibold text-slate-700 dark:text-slate-300">Paguyuban Bani P3N Kedungbanteng ©2026</p>
              <p className="text-[8px] text-slate-400 dark:text-slate-500">Kresno Gadhing Pramudhyo | Hak Cipta</p>
            </div>
          </div>
          ...
          {onLogout && (
            <button
              type="button"
              onClick={onLogout}
              className="w-full flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-xl border border-rose-200 dark:border-rose-800/80 bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 font-bold text-[11px] hover:bg-rose-100 transition-all cursor-pointer"
            >
