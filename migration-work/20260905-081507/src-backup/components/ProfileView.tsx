...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  const uploadProfileImage = async (
    ...
    try {
      ...
      console.error('Cloudinary upload gagal:', error);
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await uploadProfileImage(file, 'paguyuban-bani-p3n/logo', (url) => {
      const updated = {
        ...profile,
        logoUrl: url,
      };
  ...
  // Helper untuk mempertahankan fungsi transparansi gambar lama.
  // Saat ini upload langsung ke Cloudinary sehingga URL hasil upload
  // dapat disimpan di Firestore.
  const processImageToTransparentPNG = (
    base64: string,
  ...
  const handleSignatureUpload = async (
    ...
    await uploadProfileImage(
      file,
      'paguyuban-bani-p3n/signatures/chairman',
      (url) => {
        setEditForm((prev) => ({
          ...prev,
  ...
  const handleKopUpload = async (
    ...
    await uploadProfileImage(
      file,
      'paguyuban-bani-p3n/kop-surat',
      (url) => {
        setEditForm((prev) => ({
          ...prev,
          officialDocumentConfig: {
            ...prev.officialDocumentConfig,
            kopSuratImageUrl: url,
  ...
  const handleTreasurerSignatureUpload = async (
    e: React.ChangeEvent<HTMLInputElement>
  ) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    await uploadProfileImage(
      file,
      'paguyuban-bani-p3n/signatures/treasurer',
      (url) => {
        setEditForm((prev) => ({
          ...prev,
          officialDocumentConfig: {
            ...prev.officialDocumentConfig,
            treasurerSignatureImageUrl: url,
          },
        }));
      }
    );
  ...
  const handleDirectStampUpload = async (
    e: React.ChangeEvent<HTMLInputElement>
  ) => {
    const file = e.target.files?.[0];
    if (!file) return;
