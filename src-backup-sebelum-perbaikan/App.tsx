...
export default function App() {
  ...
  const handleSelectTab = useCallback((tab: TabType) => {
    if (currentUser?.role === 'user') {
      // Member can only access user_portal, prayer_times, and profile
      if (tab !== 'user_portal' && tab !== 'prayer_times' && tab !== 'profile') {
        setActiveTab('user_portal');
        return;
      }
    }
  ...
  useEffect(() => {
    ...
    // Both Admin & Member: Listen to Lottery Winners & Paguyuban Profile
  ...
  const handleTogglePayment = async (memberId: string, month: number, type: 'arisan' | 'iuran') => {
    ...
    setPayments((prev) => {
      let existing = prev.find((p) => p.memberId === memberId);
      let list = [...prev];
      if (!existing) {
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 lg:pl-72">
          ...
          <main className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto">
            ...
            {(activeTab === 'user_portal' || (currentUser.role === 'user' && activeTab !== 'prayer_times' && activeTab !== 'profile')) && (
              <UserDashboardView
                currentUser={currentUser}
                members={members}
                payments={payments}
