...
export const ArisanView: React.FC<ArisanViewProps> = ({
  ...
  const [selectedMonthFilter, setSelectedMonthFilterState] = useState<number>(() => {
    const saved = localStorage.getItem('arisan_p3n_arisan_month_filter_v1');
    const month = Number(saved);
    return month >= 1 && month <= 12 ? month : 5;
  });

  const setSelectedMonthFilter = useCallback((month: number) => {
    setSelectedMonthFilterState(month);
    localStorage.setItem('arisan_p3n_arisan_month_filter_v1', String(month));
  }, []);
