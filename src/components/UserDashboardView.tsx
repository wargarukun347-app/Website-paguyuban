...
export const UserDashboardView: React.FC<UserDashboardViewProps> = ({
  ...
  const currentMember =
    // 1. memberId adalah identitas kanonik.
    (currentUser.memberId
      ? members.find((m) => m.id === currentUser.memberId)
      : undefined) ||

    // 2. userId = Firebase Auth UID, untuk migrasi akun lama.
    members.find(
      (m) => (m as any).userId === currentUser.id
    ) ||

    // 3. Email hanya fallback migrasi.
    (currentUser.email
      ? members.find(
          (m) =>
            typeof (m as any).email === 'string' &&
            (m as any).email.toLowerCase().trim() ===
              currentUser.email.toLowerCase().trim()
        )
      : undefined) ||

    // 4. Fallback jika data anggota belum tersedia.
    {
      id: currentUser.memberId || `mem-${currentUser.id || 'usr'}`,
      no: members.length + 1,
